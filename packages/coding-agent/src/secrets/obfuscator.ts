...
export class SecretObfuscator {
	...
	obfuscate(text: string): string {
		...
		let result = text;
		// Placeholders already present in the INPUT were generated by a PRIOR
		// obfuscate() call. The SDK obfuscates messages in both convertToLlm and
		// transformProviderContext, and prior-turn messages re-enter every turn, so
		// obfuscate() must be a fixed point. Treat input placeholders as atomic: a
		// regex must not re-redact around an already-emitted placeholder, which would
		// corrupt a prior replacement marker (e.g. REDACTED -> REDACTEDDACTED).
		const inputPlaceholders = this.#collectInputPlaceholders(text);
		...
		for (const entry of this.#regexEntries) {
			...
			const matches = this.#collectRegexMatches(result, entry.regex, entry.mode, inputPlaceholders);
	...
	#collectInputPlaceholders(text: string): ReadonlySet<string> {
		const placeholders = new Set<string>();
		for (const range of this.#knownPlaceholderRanges(text)) {
			placeholders.add(text.slice(range.start, range.end));
		}
		return placeholders;
	}
	...
	#collectRegexMatches(
		...
		for (;;) {
			...
			// (generated by a prior obfuscate() call). Re-redacting around it would
			// corrupt already-emitted output; the prior call already handled this span.
			if (
				knownPlaceholderRanges.some(
					range =>
						start < range.end && end > range.start && inputPlaceholders.has(text.slice(range.start, range.end)),
				)
			) {
				continue;
			}
